  ...
  return (
    <section className="py-24 md:py-32 px-6 md:px-12" id="contact" aria-label="Contact">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 max-w-2xl mx-auto mt-12">
          {[
            ...
            <ScrollReveal key={item.title} delay={0.08 * i}>
              <a
                ...
                className="block bg-white/[.035] border border-white/[.07] rounded-xl p-5 text-center transition-all duration-400 hover:border-gold/30 hover:-translate-y-1.5 hover:bg-gold-dim group"
              >
                <div className="w-11 h-11 bg-gold-dim rounded-lg flex items-center justify-center text-gold text-base mx-auto mb-3 transition-all duration-300 group-hover:bg-gold group-hover:text-black">
                  <i className={item.icon} />
                </div>
                <h3 className="font-display text-sm font-bold">{item.title}</h3>
                <p className="text-[0.7rem] text-[#5a5a5a] mt-1">{item.subtitle}</p>
              </a>
            </ScrollReveal>
          ))}
        </div>
